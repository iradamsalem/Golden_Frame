...
const purposes = [
  {
    ...
    icon: (
      <Svg viewBox="0 0 24 24" width={24} height={24} fill="#E2B64D">
...
const PurposeSelector = () => {
  ...
  const handlePurposeSelect = (item) => {
    ...
    if (!photos || photos.length === 0) {
      Alert.alert('Error', 'No photos found to upload.');
      return;
    }
  ...
  if (isLoading) {
    return (
      <View style={[styles.container, { justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ color: 'gold', fontSize: 18 }}>Loading user info...</Text>
      </View>
    );
  }
