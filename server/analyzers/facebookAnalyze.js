export const analyzeFacebook = (photoScoresMap) => {
  const weights = {
    resolution: 0.20,
    variance: 0.15,
    face: 0.15,
  ...
  const preferredRGB = [
    ...
    { red: 0, green: 128, blue: 0 },     // #008000
    { red: 169, green: 169, blue: 169 }, // #A9A9A9
  ...
  const evaluateColorsForFacebook = (colors) => {
  ...
  for (const [photoName, scores] of photoScoresMap.entries()) {
    ...
    const colorScoreValue = evaluateColorsForFacebook(scores.colors);
    ...
    console.log(`📘 Facebook Photo: ${photoName}`);
    console.log(`   Resolution: ${scores.resolution}, Expression: ${scores.expression}, ColorScore: ${colorScoreValue}, LabelScore: ${labelScore}`);
